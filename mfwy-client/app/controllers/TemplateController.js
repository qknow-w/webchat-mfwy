...
define(['app'], function (app) {
    app.controller('TemplateController', ['$scope','templateService',"$location",'$stateParams', function ($scope,templateService,$location,$stateParams) {
        var dingdan,text,tplid;
        $scope.address={};
        $scope.division = {
        ...
        $scope.tijiao=function(){
            templateService.order(dingdan).then(function (data) {
                alert("订单提交成功");
                $location.path("/template/zfdd");
            });
        };
        ...
        $scope.addOrder = function () {
            templateService.getWechatConfig().then(function(data){
                alert(data);
        ...
        $scope.jia=function(){
            ...
            templateService.xiadan(tplid).then(function(data){
                $scope.tplprice=data.price;
                dingdan.temInfo.id=tplid;
            });
            dingdan.temInfo.type=$scope.papertype;
            dingdan.temInfo.gongyi=$scope.cardgongyi;
            var gongyiprice = parseInt($(".gongyi").find("option:selected").attr("data-gongyiprice"));
            var typeprice=parseInt($(".type").find("option:selected").attr("data-typeprice"));
            if(isNaN(typeprice)||isNaN(gongyiprice)){
                $scope.danjia=0.00;
            }else{
                $scope.danjia=(gongyiprice+typeprice)*parseInt($scope.order.num)+parseInt($scope.tplprice);
            }
            dingdan.totalMoney=$scope.dingdan;
        };

        var tpl=function(){
            templateService.tpl().then(function(data) {
                $scope.tpls=data;
            });
        };
        tpl();
