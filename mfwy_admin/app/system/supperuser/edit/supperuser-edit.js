 ...
 */
angular.module("supperuser-edit", ["resource.superusers"]).config([
    "$routeProvider", function ($routeProvider) {
        return $routeProvider.when("/system/supperuser/new", {
            templateUrl: "/app/system/supperuser/edit/supperuser-edit.tpl.html",
            controller: 'SupperuserEditCtrl',
            resolve: {
                supperuser: function () {
                    return ac = {
                        name:"",//昵称
                        ...
                        password:"",//密码
                        currentAdd:"",//当前位置标识，
                        AddressInfo:{name:"",province:"",city:"",district:""},//用户地址信息 省 市 区
                        role:""//角色
                    };
                }
            }
        }).when("/system/supperuser/:id", {
            templateUrl: "/app/system/supperuser/edit/supperuser-edit.tpl.html",
            controller: 'SupperuserEditCtrl',
            resolve: {
                supperuser: [
                    "$q", "$route", "SuperUsers", function ($q, $route, SuperUsers) {
                        var deferred;
                        deferred = $q.defer();
                        SuperUsers.get({
    ...
    .controller("SupperuserEditCtrl", ["$scope", "$routeParams", "$location", "$rootScope","$http",'Sites', "SuperUsers", "supperuser", "messager",
        function ($scope, $routeParams, $location, $rootScope,$http,Sites,SuperUsers, supperuser, messager) {
            $scope.get = function () {
                $scope.entity = supperuser;
            ...
            save = function () {
                ...
                $scope.loading = "Saving";
                entity = $scope.entity;
                $scope.siteData.forEach(function(value,index){
                   if(value.id==entity.currentAdd){
                       entity.AddressInfo.name=value.name;
                   }
                });
                if (!$routeParams.id) {
                    if(entity.currentAdd==""){
                        entity.currentAdd=0;
                        entity.role="超级管理员"
                    }else{
                        entity.role="管理员"
                    ...
                    return SuperUsers.post(entity, function (data) {
                        messager.success("save successfully.");
                        return $location.path("/system/supperuser");
                    });
                } else {
                    return SuperUsers.put({
                        id: "" + entity.id
